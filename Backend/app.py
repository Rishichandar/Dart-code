...
def train_and_predict():
    try:
        ...
        if not all(isinstance(row, list) and len(row) == len(columns) for row in rows):
            raise ValueError("All rows must be arrays and have the same number of elements as columns")
...
def balance():
    try:
        ...
        balanced_data = {
            'columns': X_resampled.columns.tolist() + [target_column],
            'data': pd.concat([X_resampled, y_resampled], axis=1).to_json(orient='records')  # Convert DataFrame to JSON
        }
        print("Balanced Data:", balanced_data)  # Debugging
        print("X_resampled.values:", pd.concat([X_resampled, y_resampled], axis=1).to_json(orient='records'))  # Debugging
