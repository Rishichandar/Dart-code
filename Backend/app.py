...
#         if df is None:
#             return 'Unsupported file type or error reading the file', 400

#         # Get the selected fill method and constant value
#         fill_method = request.form['method']
#         constant = request.form.get('constant', default=0, type=float)

#         # Process the DataFrame to handle missing values
#         processed_df = preprocess.poz_handle_missing_values(df, fill_method, constant)

#         if processed_df.empty:
#             return 'Error processing the DataFrame', 500

#         # Convert the processed DataFrame to HTML for display
#         processed_data_html = processed_df.to_html()
#         return render_template('index.html', processed_data=processed_data_html)
...
def handle_missing_values():
    try:
        ...
        df = preprocess.poz_read_file(file)
        if df is None:
            return jsonify({'error': 'Unsupported file type or error reading the file'}), 400
        ...
        if processed_df.empty:
            return jsonify({'error': 'Error processing the DataFrame'}), 500

        processed_data_json = processed_df.to_json(orient='records')
        return jsonify({'processed_data': processed_data_json})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
